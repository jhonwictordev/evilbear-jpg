...
function Hero({ t }: { t: typeof copy.pt }) { return <section className="hero section-shell" id="home"><div className="hero-copy"><p className="eyebrow"><i />{t.heroEyebrow}</p><h1><img src="/evilbear-logo.webp" alt={t.heroTitle} /></h1><p className="hero-role">{t.heroRole}</p><p className="hero-text">{t.heroText}</p><div className="hero-actions"><a className="button button--primary" href="#work">{t.work} <span>↓</span></a><a className="button" href="#contact">{t.start} <Arrow /></a></div></div><div className="hero-art"><img src="/mascot.png" alt="" /></div><span className="scroll-note">{t.scroll}</span></section>; }
