...
const attempts = globalThis.__evilbearCheckoutAttempts ?? new Map();
globalThis.__evilbearCheckoutAttempts = attempts;
...
export default async function handler(req, res) {
  ...
  if (!withinRateLimit(req)) return sendJson(res, { message: "Muitas tentativas. Aguarde um minuto." }, 429);
  const accessToken = process.env.MERCADO_PAGO_ACCESS_TOKEN;
  const webhookSecret = process.env.MERCADO_PAGO_WEBHOOK_SECRET;
  if (!accessToken || !webhookSecret) {
    return sendJson(res, { message: "Checkout indisponível. A integração de pagamentos ainda não foi ativada." }, 503);
  }
  ...
  try {
    ...
    return sendJson(res, { message: "Dados de checkout inválidos." }, 400);
  }
  ...
  if (name.length < 2 || !validEmail(email) || items.length === 0) {
    return sendJson(res, { message: "Revise seus dados e itens do carrinho." }, 400);
  }
  ...
  const siteUrl = publicOrigin(req);
  const preference = {
  ...
  if (!response.ok || !payment?.init_point) {
    ...
    await saveOrder(order);
    return sendJson(res, { message: "Não foi possível iniciar o pagamento. Tente novamente." }, 502);
  }

  order.preferenceId = payment.id;
  order.updatedAt = new Date().toISOString();
  await saveOrder(order);
  return sendJson(res, { orderNumber: order.orderNumber, initPoint: payment.init_point });
}
